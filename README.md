...
`npm run dev` - запускает dev сборку проекта с помощью browser-sync, позволяя проверять mobile версию проекта, находясь в одной интернет-сети
...
Благодаря плагину __gulp-file-include__ можно разделять html-файл на различные шаблоны, которые должны храниться в папке __partials__. Удобно делить html-страницу на секции.

> Для вставки html-частей в главный файл необходиммо использовать `@include('partials/filename.html')`
